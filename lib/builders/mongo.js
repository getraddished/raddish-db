        ...
        for(var where of wheres) {
            ...
            if(where.column === '_id') {
                if(Array.isArray(where.value)) {
                    where.value = where.value.map(function(value) {
            ...
            case 'like':
                ...
                break;
            case 'between':
                query[where.column] = {$gte: where.value.start, $lte: where.value.end};
                break;
            case 'gt':
                query[where.column] = {$gte: where.value};
                break;
            case 'lt':
                query[where.column] = {$lte: where.value};
                break;
            }
        }
    }
