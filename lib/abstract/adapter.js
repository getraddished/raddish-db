...
AbstractAdapter.prototype.getBuilder = function(type) {
    var builder = this.builder,
        accepted_types = ['select', 'insert', 'update', 'delete'];

    if(!type || accepted_types.indexOf(type) == -1) {
        throw new TypeError('This query type (' + type + ') isn\'t supported (yet)!');
    }

    return new (require(__dirname + '/../' + this.type + '/builder/' + type));
};
